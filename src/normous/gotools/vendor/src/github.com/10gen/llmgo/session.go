...
type MongoSession interface {
	AcquireSocketPrivate(subordinateOk bool) (*MongoSocket, error)
}
...
func (s *Session) AcquireSocketPrivate(subordinateOk bool) (*MongoSocket, error) {
...
func (c *Collection) writeOp(op interface{}, ordered bool) (lerr *LastError, err error) {
	...
	socket, err := s.AcquireSocketPrivate(c.Database.Name == "local")
	if err != nil {
		return nil, err
	}
	defer socket.Release()
